...
public class CircuitLockerClone {
	...
	void checkForWritePermission(String operationName) {
		if (mutatingThread != Thread.currentThread()) {
			...
			if(false) System.out.println("Allowing write Permission!"); // I left this here as a reminder.
		}
	}
	...
	/*static Map<CircuitClone,Lock> acquireLocks(CircuitTransaction xn,
			CircuitMutatorImpl mutator) {
		CircuitTransaction xnClone = xn.
		Map<CircuitClone,Integer> requests = xn.getAccessedCircuits();
		Map<CircuitClone,Lock> circuitLocks = new HashMap<CircuitClone,Lock>();
		// Acquire locks in serial-number order to avoid deadlock
		CircuitClone[] lockOrder = requests.keySet().toArray(new CircuitClone[0]);
		Arrays.sort(lockOrder, new CircuitComparator());
		try {
			for (CircuitClone circ : lockOrder) {
				Integer access = requests.get(circ);
				CircuitLockerClone locker = circ.getLocker();
